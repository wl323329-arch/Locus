...
function PlotCanvas({
  ...
  const implicitCurveSamples = useMemo(() => (
    sampleVisibleImplicitCurves(curveEntities, worldBounds, implicitGrid.cols, implicitGrid.rows, parameters)
  ), [curveEntities, implicitGrid.cols, implicitGrid.rows, parameters, worldBounds]);
  ...
  const implicitCurves = useMemo(() => (
    implicitCurveSamples
      .map((fn) => {
        const segments = fn.segments
          .map((segment) => {
            const aScreen = worldToScreen(segment.ax, segment.ay);
            const bScreen = worldToScreen(segment.bx, segment.by);
            return {
              ...segment,
              asx: aScreen[0], asy: aScreen[1],
              bsx: bScreen[0], bsy: bScreen[1],
            };
          })
          .filter((segment) => (
            Math.max(segment.asx, segment.bsx) >= -24 &&
            Math.min(segment.asx, segment.bsx) <= size.w + 24 &&
            Math.max(segment.asy, segment.bsy) >= -24 &&
            Math.min(segment.asy, segment.bsy) <= size.h + 24
          ));
  ...
  ), [implicitCurveSamples, size.h, size.w, worldToScreen]);
  ...
  const tangentHandles = useMemo(() => (
    tangentPoints
      ...
      .filter((handle) => handle && handle.sx >= -20 && handle.sx <= size.w + 20 && handle.sy >= -20 && handle.sy <= size.h + 20)
  ), [functions, size.h, size.w, tangentPoints, worldToScreen]);
  ...
  const curveTracker = useMemo(() => {
    ...
    sampledPolylineCurves
      .filter((fn) => fn.compiled.kind !== 'explicit')
      .forEach((fn) => {
  ...
  const onMouseMove = (e) => {
    ...
    if (tangentDragRef.current) {
      ...
      if (curvePoint) {
        onTangentPointsChange((prev) => prev.map((tangent) => (
          tangent.id === tangentDragRef.current.id
...
function FunctionRow({ fn, theme, onChange, onDelete, isOnly, selected, onSelect }) {
  ...
  const descriptor = curveKind === 'implicit'
    ? `${label}(x, y)`
  ...
  return (
    <div style={{ padding: '10px 16px', borderBottom: `1px solid ${theme.rule}` }}>
      <div style={{
        ...
        marginBottom: 6, fontFamily: '"JetBrains Mono", monospace', fontSize: 11,
      }}>
        <span style={{ color: theme.ink }}>{name}</span>
        <span style={{ color: theme.muted }}>{formatDecimalNumber(config.value)}</span>
      </div>
      <input
        type="range"
        min={config.min}
