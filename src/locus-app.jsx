...
function App() {
  ...
  useEffect(() => { try { localStorage.setItem('locus:parameters', JSON.stringify(parameterConfig)); } catch {} }, [parameterConfig]);
  useEffect(() => { try { localStorage.setItem('locus:functions', JSON.stringify(functions)); } catch {} }, [functions]);
  useEffect(() => { try { localStorage.setItem('locus:tangentPoints', JSON.stringify(tangentPoints)); } catch {} }, [tangentPoints]);
  ...
  useEffect(() => {
    setParameterConfig((prev) => {
      ...
      if (
        prevKeys.length === parameterNames.length &&
        prevKeys.every((key) => next[key] && JSON.stringify(next[key]) === JSON.stringify(prev[key]))
      ) return prev;
      return next;
    });
  }, [parameterNames]);
      ...
      const next = prev
        .map((tangent) => {
          ...
          if (fn.compiled.kind === 'parametric' || fn.compiled.kind === 'polar') {
            ...
            return (
              Math.abs(point.x - tangent.x) > 1e-6 ||
              Math.abs(point.y - tangent.y) > 1e-6
            ) ? { ...tangent, x: point.x, y: point.y } : tangent;
          }
          return tangent;
        })
        .filter(Boolean);
  ...
  return (
    <div style={{ display: 'flex', width: '100%', height: '100%', background: theme.bg }}>
      ...
      <main style={{ flex: 1, position: 'relative', background: theme.bg, minWidth: 0 }}>
        ...
        />
        <div style={{ position: 'absolute', top: 16, right: 16, display: 'flex', gap: 8, alignItems: 'flex-start' }}>
            ...
            {tweaksOpen && (
              <div ref={tweaksRef} className="tweaks-panel">
                    ...
                    >
                      <div style={{ position: 'absolute', inset: 5, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 2 }}>
                        {t.funcPalette.slice(0, 3).map((color, idx) => (
                          <div key={idx} style={{ background: color, borderRadius: 2 }}/>
                        ))}
                      </div>
                    </button>
                  ))}
                ...
                <div style={{ marginTop: 10, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 6 }}>
                  {Object.entries(window.LOCUS_THEMES).map(([key, t]) => (
                    <div key={key} className="theme-swatch-label">{t.name}</div>
                  ))}
                </div>
              </div>
            )}
